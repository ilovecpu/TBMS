...
const SALES_ORDERS_HEADERS = ['id','branch','branchName','orderNumber','timestamp','date','orderType','paymentMethod','total','itemCount','items','refunded','refundedAt','refundMethod','vat','cashPct','vatablePct','pushedAt','refundedBy'];
...
function doGet(e) {
  try {
    ...
    switch(action) {
      ...
      case 'ping':     result = {status:'ok', time: new Date().toISOString(), version:'TBMS 4.5.8'}; break;
...
function _getOrCreateSOSheet(sheetName, ss) {
  ...
  if (!sh) {
    ...
    // ★ 기존 시트 헤더 마이그레이션: 컬럼 수가 부족하면 새 컬럼 추가 (끝에만 추가되므로 안전)
    var existingCols = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
    if (existingCols.length < SALES_ORDERS_HEADERS.length) {
      var missing = SALES_ORDERS_HEADERS.slice(existingCols.length);
      var startCol = existingCols.length + 1;
      sh.getRange(1, startCol, 1, missing.length).setValues([missing]);
      sh.getRange(1, startCol, 1, missing.length).setFontWeight('bold');
      Logger.log('[SalesOrders] Migrated sheet ' + sheetName + ': added columns ' + missing.join(', '));
    }
  }
  return sh;
}
